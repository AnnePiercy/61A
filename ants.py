...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(insect, QueenAnt) and insect.queen: return
            # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        bees = bee_place.bees
        k = 0
        while len(bees) == 0 and bee_place.entrance != hive:
            bee_place = bee_place.entrance
            bees = bee_place.bees
            k += 1

        if self.min_range <= k and k <= self.max_range:
...
# END Problem 13
    ...
    def __init__(self):
        ...
        if not QueenAnt.onequeen:
            QueenAnt.onequeen = True
            self.queen = True
        else: self.queen = False
        QueenAnt.effected_ants.append(self)
        # END Problem 13
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        if self.queen == False: self.reduce_armor(self.armor)
        else:
            location = self.place
            while location != None:
                if location.ant != None and location.ant.container and location.ant.ant != None and location.ant.ant not in QueenAnt.effected_ants:
                    location.ant.ant.damage *=  2
                    QueenAnt.effected_ants.append(location.ant.ant)
                if location.ant != None and location.ant not in QueenAnt.effected_ants:
                    location.ant.damage *= 2
                    QueenAnt.effected_ants.append(location.ant)
                location = location.exit
            return ScubaThrower.action(self, colony)
        # END Problem 13
